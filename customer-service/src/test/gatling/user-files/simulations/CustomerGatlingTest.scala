...
class CustomerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Customer entity")
        ...
        .repeat(2) {
            ...
            .get("/customerservice/api/customers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "code":"SAMPLE_TEXT", "name":"SAMPLE_TEXT", "tradingName":"SAMPLE_TEXT", "abn":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_customer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created customer")
                .get("/customerservice${new_customer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created customer")
            .delete("/customerservice${new_customer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
