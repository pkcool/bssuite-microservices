...
public class CustomerCategoryResource {
    ...
    private final CustomerCategoryQueryService customerCategoryQueryService;

    public CustomerCategoryResource(CustomerCategoryService customerCategoryService, CustomerCategoryQueryService customerCategoryQueryService) {
        this.customerCategoryService = customerCategoryService;
        this.customerCategoryQueryService = customerCategoryQueryService;
    }
     ...
     * or with status 500 (Internal Server Error) if the customerCategoryDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    @Timed
    public ResponseEntity<List<CustomerCategoryDTO>> getAllCustomerCategories(CustomerCategoryCriteria criteria,@ApiParam Pageable pageable) {
        log.debug("REST request to get CustomerCategories by criteria: {}", criteria);
        Page<CustomerCategoryDTO> page = customerCategoryQueryService.findByCriteria(criteria, pageable);
