...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/customer-category'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId (index: number, item: CustomerCategory) {
        return item.id;
    }
    ...
    registerChangeInCustomerCategories() {
        this.eventSubscriber = this.eventManager.subscribe('customerCategoryListModification', (response) => this.loadAll());
    }
